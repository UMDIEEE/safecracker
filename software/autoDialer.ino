...
#define DEBUGDELAY(x)   (DEBUG_DELAY) ? delay(x) :
...
#define ONBOARD_LED     13 // onboard LED on pin 13
...
////////// objects ///////////
SoftwareSerial LCD(10, 11); // LCD object, Arduino SS_RX = pin 10 (unused, no need to recieve data from LCD), Arduino SS_TX = pin 11
...
////////////////////////////////////////
/*
 interrupt hander (ISR) called on falling edge of photogate pin
 
 increments opened, toggles tripped global variables
 recalibrates library position variable
 */
void onZeroTriggered()
{
    ...
    digitalWrite(ONBOARD_LED, tripped);

    //if the photogate is being tripped by the toZero()
    //function (assuming the toZero() function is called
    //prior to any other movements of the motor)
    if(!beenTriggeredBefore)
    {
        //have the motor come to a stop. You MUST then call run()
        //so the motor can physically decelerate to 0.
        stepper.move(0);
...
/*
 This function will update the global 'pos' variable to reflect where
 the new position should be after the motor rotates delta digits on the dial.
  delta is the number of digits on the dial you wish to rotate.
 */
static inline void updatePosition(int32_t delta)
{
    ...
    //if (pos - delta) is negative, then modulus will return a negative number (in C and Java).
    //We need a positive number. ex: -3 mod 100 will return -3. Need it to return 97.
    if( pos < 0 )
    {
        pos = pos + NUM_DIGITS;
    }
...
void toZero()
{
    ...
    {
        /*
         this condition is here so that if the above condition in the while loop
         is not being evaluated at the precise moment when the photogate is blocked,
         we will still be able to jump out of the loop and return as long as the ISR has been called.
         */
        if(beenTriggeredBefore)
        {
            pos = 0;
        ...
        /*
         have the function quit once it gets to the beginning of the while loop again.
         We only want this function to rotate 1.1 times times once. If photogate isn't tripped,
         something is wrong (or safe has been cracked). We don't want to be stuck in this loop
         if beenTriggeredBefore never gets set to one in the IRS.
         */
        quit = 1;
    }
...
void tryCombo( int32_t c0, int32_t c1, int32_t  c2 )
{
    ...
    */
    ...
    //going CCW to first digit. argument to stepper.move() better be positive.
    ...
    */
    ...
    //going CW to second digit. argument to stepper.move() better be negative.
    ...
    */
    ...
    //going CCW to third digit. Argument to stepper.move() better be positive.
    ...
    */
    ...
    /*
     if the third digit is 0, then rotating around one REV at the end will increment opened to 1.
     And then going toZero() won't increment opened to 2 b/c the dial is already at 0.
     So increment opened by 1. Now, if it's still less than 2, we know that the dial never made it back to 0.
     */
    if(c2 == 0)
    {
        opened++;
...
void setup()
{
    ...
    //associate pin 2 on UNO with interrupt function.
    //onZeroTriggered gets called whenever pin 2 goes low.
    //connect the SIG terminal of photogate to pin 2 on the UNO.
    attachInterrupt(digitalPinToInterrupt(INTERRUPT_PIN), onZeroTriggered, FALLING);
...
void loop()
{
    ...
    toZero();
    //todo: add calibration function here
    delay(3000);
